---
title: Next.js 13 要点
description: Next.js 13 Beta ドキュメントの要点をまとめてみた。
date: '2022-10-30'
---

この記事は、[Next.js 13 Beta Docs](https://beta.nextjs.org/docs) の要点をまとめて日本語に翻訳したものです（一部、他ドキュメントの情報あり）。翻訳ツールの出力をそのまま使用しているので、不自然な表現があるかもしれません。どうぞご了承ください。
...
[The `app` Directory](https://beta.nextjs.org/docs/routing/fundamentals#the-app-directory)

### Special Files

`js`、`jsx`、または `tsx` ファイル拡張子は、特別なファイルに使用できます。`ts` は現在サポートされていません。

[Special Files](https://beta.nextjs.org/docs/routing/fundamentals#special-files)
...
[Server-Centric Routing with Client-side Navigation](https://beta.nextjs.org/docs/routing/fundamentals#server-centric-routing-with-client-side-navigation)
...
[Partial Rendering](https://beta.nextjs.org/docs/routing/fundamentals#partial-rendering)
...
[Pages](https://beta.nextjs.org/docs/routing/pages-and-layouts#pages)
...
[Layouts](https://beta.nextjs.org/docs/routing/pages-and-layouts#layouts)

#### Root Layout (必須)

- `app` ディレクトリには、ルート レイアウトが含まれている必要があります。
- Next.js が自動的に作成しないため、ルート レイアウトは `<html>` および `<body>` タグを定義する必要があります。`<head>` タグはオプションです。
...
[Nesting Layouts](https://beta.nextjs.org/docs/routing/pages-and-layouts#nesting-layouts)
...
[Templates](https://beta.nextjs.org/docs/routing/pages-and-layouts#templates)

### Modifying `<head>`

`app` ディレクトリで、新しい特別な `head.js` ファイルを使用して `<head>` タグを変更できます。これは、`pages` ディレクトリの `next/head` を置き換えます。

[Modifying `<head>`](https://beta.nextjs.org/docs/routing/pages-and-layouts#modifying-head)
...
[Component-level Client and Server Rendering](https://beta.nextjs.org/docs/rendering/fundamentals#component-level-client-and-server-rendering)
...
[Static Rendering](https://beta.nextjs.org/docs/rendering/fundamentals#static-rendering)
...
[Dynamic Rendering](https://beta.nextjs.org/docs/rendering/fundamentals#dynamic-rendering)
...
[Server Components](https://beta.nextjs.org/docs/rendering/server-and-client-components#server-components)
...
[Data Fetching](https://beta.nextjs.org/docs/rendering/server-and-client-components#data-fetching)
...
[Passing props from Server to Client Components (Serialization)](https://beta.nextjs.org/docs/rendering/server-and-client-components#passing-props-from-server-to-client-components-serialization)
...
[Keeping Server-Only Code out of Client Components (Poisoning)](https://beta.nextjs.org/docs/rendering/server-and-client-components#keeping-server-only-code-out-of-client-components-poisoning)
...
[Moving Client Components to the Leaves](https://beta.nextjs.org/docs/rendering/server-and-client-components#moving-client-components-to-the-leaves)
...
[Importing Server Components into Client Components](https://beta.nextjs.org/docs/rendering/server-and-client-components#importing-server-components-into-client-components)
...
[Fetching Data with Server Components](https://beta.nextjs.org/docs/data-fetching/fundamentals#fetching-data-with-server-components)
...
[Component-level Data Fetching](https://beta.nextjs.org/docs/data-fetching/fundamentals#component-level-data-fetching)
...
[Streaming and Suspense](https://beta.nextjs.org/docs/data-fetching/fundamentals#streaming-and-suspense)
...
[Example: `fetch` and `async`/`await` in Server Components](https://beta.nextjs.org/docs/data-fetching/fetching#example-fetch-and-asyncawait-in-server-components)
...
`fetch` は現在クライアント コンポーネントではサポートされておらず、複数の再レンダリングをトリガーする可能性があります。現時点では、クライアント コンポーネントでデータを取得する必要がある場合は、[SWR](https://swr.vercel.app/) などのサードパーティ ライブラリを使用することをお勧めします。

`use` は `await` と概念的に似た promise を受け入れる新しい React 関数です。`use` は、コンポーネント、フック、およびサスペンスと互換性のある方法で、関数によって返される promise を処理します。

[Example: `fetch` and `use` in Client Components](https://beta.nextjs.org/docs/data-fetching/fetching#example-fetch-and-use-in-client-components)
...
[Static Data](https://beta.nextjs.org/docs/data-fetching/fetching#static-data)
...
[Revalidating Data](https://beta.nextjs.org/docs/data-fetching/fetching#revalidating-data)
...
[Dynamic Data](https://beta.nextjs.org/docs/data-fetching/fetching#dynamic-data)
...
[Blocking Rendering in a Route](https://beta.nextjs.org/docs/data-fetching/fetching#blocking-rendering-in-a-route)
...
[Instant Loading UI](https://beta.nextjs.org/docs/data-fetching/streaming-and-suspense#instant-loading-ui)
...
[Suspense Boundaries](https://beta.nextjs.org/docs/data-fetching/streaming-and-suspense#suspense-boundaries)
...
[https://beta.nextjs.org/docs/styling/tailwind-css](https://beta.nextjs.org/docs/styling/tailwind-css)

## OG Image Generation

[https://vercel.com/docs/concepts/functions/edge-functions/og-image-generation](https://vercel.com/docs/concepts/functions/edge-functions/og-image-generation)
